...
async function open(page: Page, summaryId: string) {
  await page.goto(`${baseUrl}/visual?summary=${encodeURIComponent(summaryId)}`, { waitUntil: 'networkidle' });
  await dismissWelcome(page);
  await page.getByRole('img', { name: /Mapa de relações/ }).waitFor();
}
...
await open(desktop, 'fis-termologia-calor');
await desktop.locator('.vs-concept-card').first().click();
await desktop.screenshot({ path: path.join(output, 'fisica-calor-desktop.png'), fullPage: true });

await open(desktop, 'bio-ecologia-introducao');
await desktop.screenshot({ path: path.join(output, 'biologia-ecologia-desktop.png'), fullPage: true });

await open(desktop, 'summary-historia-antiguidade-classica-o-mundo-grego');
await desktop.screenshot({ path: path.join(output, 'historia-mundo-grego-desktop.png'), fullPage: true });

const mobile = await browser.newPage({ viewport: { width: 375, height: 812 }, deviceScaleFactor: 1 });
monitor(mobile, 'mobile');
await open(mobile, 'fis-termologia-calor');
...
await open(reducedPage, 'bio-ecologia-introducao');
await reducedPage.locator('.vs-topic-artifact').waitFor();
