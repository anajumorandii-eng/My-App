...
O checkout integral materializou `public/questions.json` e `public/flashcards/biologia.json`, mas `npm test` ainda falha no teste do catálogo por 14 PDFs de apostilas ausentes em `materiais brutos/`. Após as correções desta rodada, `npm run lint`, `npm run build`, `git diff --check` e os **17 testes direcionados** passaram. A execução ampla do Vitest com um worker avançou por vários minutos e foi interrompida sem resultado final; a CI da PR precisa validar a suíte completa na revisão publicada. Os checks remotos da revisão anterior `02de4cb` passaram.
