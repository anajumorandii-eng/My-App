...
describe('Contagem honesta de cenas-âncora', () => {
  it('conta só entradas com lastro, somando todas as matérias', () => { expect(entradasValidas().length).toBe(filosofia.length + sociologia.length + historia.length + geografia.length + literatura.length + matematica.length); });
  it('não conta lacunas declaradas', () => {
    const ids = new Set(entradasValidas().map((e) => e.chapterId));
    for (const gap of [...filosofiaSemCena, ...sociologiaSemCena, ...historiaSemCena, ...geografiaSemCena, ...literaturaSemCena, ...matematicaSemCena]) expect(ids.has(gap.chapterId), gap.chapterId).toBe(false);
  });
