...
describe('Seleção de cena por capítulo', () => {
  ...
  it('só expõe entradas com lastro, somando todas as matérias', () => { expect(entradasValidas().length).toBe(filosofia.length + sociologia.length + historia.length + geografia.length); });
});
