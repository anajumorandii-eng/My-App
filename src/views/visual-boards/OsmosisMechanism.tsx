...
export default function OsmosisMechanism() {
  ...
  const plant=70+(delta===1?13:delta===-1?-20:0)*clock.time;
  return <MechanismFrame note="Comparação qualitativa com solutos que não atravessam a membrana. Mesma pressão inicial; água cruza nos dois sentidos. Os marcadores mostram a direção das trocas; no meio isotônico, dois sentidos se equilibram. A parede limita a expansão vegetal; o desenho não simula ruptura nem prevê volumes reais." controls={<>
    ...
    <svg viewBox="0 0 520 400" role="img" aria-label={`Osmose em células animal e vegetal: meio ${labels[medium].toLowerCase()}`}>
      ...
      {[130,385].map(x=><g key={x}><path d={`M${x} 110V220`} stroke="var(--mf-blue)" strokeDasharray="5 5" />
        {delta===0 ? <><circle cx={x-8} cy={110+110*clock.time} r="7" fill="var(--mf-blue)"/><circle cx={x+8} cy={220-110*clock.time} r="7" fill="var(--mf-blue)"/></> : <circle cx={x} cy={delta===1?110+110*clock.time:220-110*clock.time} r="7" fill="var(--mf-blue)" />}
