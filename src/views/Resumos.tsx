...
import { BookOpen, Search, Star, ChevronDown, ChevronUp, Check, AlertTriangle, CloudOff, ExternalLink, ArrowLeft, Brain, Waypoints } from 'lucide-react';
...
export default function Resumos() {
  ...
  return (
    ...
    >
      ...
      <label className="flex items-center gap-2 my-4 text-sm text-[var(--text)]"><input type="checkbox" checked={onlyDeep} onChange={e => setOnlyDeep(e.target.checked)} />Somente capítulos aprofundados ({interactiveSummaries.filter(s => s.contentStatus === 'aprofundado').length})</label>
      {filtered.length === 0 ? <div className="rounded-2xl border border-dashed border-[var(--line)] py-16 text-center"><Search className="w-8 h-8 mx-auto text-[var(--dim)] mb-3"/><h2 className="font-bold text-[var(--text)]">Nenhum resumo encontrado</h2><p className="text-sm text-[var(--dim)] mt-1">Remova um filtro ou tente outro termo.</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{filtered.map((item) => { const p = progress[item.id]; return <button key={item.id} onClick={() => { setSelectedId(item.id); setSearchParams({ summary: item.id }); setExpanded(item.sections.filter((s) => s.depth === mode).slice(0, 1).map((s) => s.id)); update(item.id, (current) => ({ ...current, lastOpenedAt: new Date().toISOString() })); }} className="text-left rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5 hover:border-[var(--primary)] focus-visible:outline-none focus-visible:ring-2" style={{'--tw-ring-color': LIT_PALETTE.primary} as React.CSSProperties}><div className="flex justify-between gap-3"><span className="subject-text text-xs font-bold uppercase tracking-wide">{item.subject}</span>{p?.important && <Star className="w-4 h-4 fill-amber-400 text-amber-400"/>}</div><h2 className="font-bold text-xl mt-2 text-[var(--text)]">{item.title}</h2><p className="mt-2 text-xs font-semibold text-[var(--dim)]">{item.contentStatus === 'aprofundado' ? 'Aprofundado · exemplos e prática' : item.contentStatus === 'roteiro' ? 'Roteiro inicial · aprofundamento pendente' : 'Material de estudo'}</p><p className="text-sm text-[var(--dim)] mt-2">{item.overview}</p><div className="flex flex-wrap gap-2 mt-4"><span className="rounded-full px-2.5 py-1 text-[10px] font-mono" style={{backgroundColor: LIT_PALETTE.primary, color: PALETTE_INK}}>Fuvest · 1ª e 2ª fases</span>{item.currentAffairs && <span className="rounded-full bg-emerald-50 dark:bg-emerald-950/30 px-2.5 py-1 text-xs">A.T.U.A.L.</span>}</div><div className="mt-4 flex items-center justify-between text-xs text-[var(--dim)]"><span>{statusLabels[p?.status ?? 'nao-iniciado']}</span><span>{getReadingProgress(item, p)}% lido</span></div></button>; })}</div>}
    </div>
  );
}
