...
function ContextScene({ id, index }: { id: GeographyContextId; index: number }) {
  const config = GEOGRAPHY_CONTEXTS[id];
  const reduced = useReducedMotion();
  const marker = useId().replace(/:/g, '');
  const positions = [[62, 88], [160, 158], [258, 88]] as const;
  return <svg className="vs-plane" viewBox="0 0 320 250" role="img" aria-label={`${config.title}: ${config.cases[index].label} em foco`} data-geography-context={id}>
...
export function geographyContextInstrument(id: GeographyContextId) {
  ...
  return function GeographyContextBoard(props: BoardProps) {
    ...
    return <BoardShell kicker="Leitura territorial" title={config.title} subtitle={config.question}
      ...
      scene={<div className="vs-instrument"><ContextScene id={id} index={index} /><div className="vs-plane-controls"><div className="vs-plane-control"><p>Alterne a escala de leitura:</p><div className="vs-geography-options" role="group" aria-label={`Recortes de ${config.title}`}>{config.cases.map((item, itemIndex) => <button key={item.label} type="button" aria-pressed={index === itemIndex} onClick={() => setIndex(itemIndex)}>{item.label}</button>)}</div></div></div><dl className="vs-plane-readouts" aria-live="polite"><div><dt>Onde</dt><dd>{selected.location}</dd></div><div><dt>Observe</dt><dd>{selected.observation}</dd></div><div data-pivot="true"><dt>Conclua</dt><dd>{selected.conclusion}</dd></div></dl><p className="vs-instrument-dica">{config.caution}</p></div>}
