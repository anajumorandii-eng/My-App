...
test('generateDeferredActions retorna exatamente o que generateDailyPlan deixou de fora', () => {
  ...
  const availableMinutes = 45; // deliberadamente pequeno em relação ao total estimado da fixture

  const plan = EfficiencyEngine.generateDailyPlan(masteryData, topics, profile(), availableMinutes, NO_GOALS, now);
  const deferred = EfficiencyEngine.generateDeferredActions(masteryData, topics, profile(), availableMinutes, NO_GOALS, now);

  const planIds = new Set(plan.map((a) => a.topicId));
  const deferredIds = new Set(deferred.map((a) => a.topicId));
  assert.equal(planIds.size + deferredIds.size, masteryData.length, 'every topic should be in exactly one of plan/deferred');
